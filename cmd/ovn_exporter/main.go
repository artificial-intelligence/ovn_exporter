...
func main() {
	var collectOvsdbServerMetrics bool
	var collectVswitchMetrics bool
	var collectPidMetrics bool
	var collectLogMetrics bool
	var collectNorthboundMetrics bool
	var collectSouthboundMetrics bool
	var collectNorthdMetrics bool
	...
	flag.BoolVar(&collectOvsdbServerMetrics, "collect.ovsdb.server.metrics", true, "Collect metrics from ovsdb-server.")
	flag.BoolVar(&collectVswitchMetrics, "collect.vswitch.metrics", true, "Collect metrics from Open vSwitch.")
	flag.BoolVar(&collectPidMetrics, "collect.pid.metrics", true, "Collect metrics from process IDs.")
	flag.BoolVar(&collectLogMetrics, "collect.log.metrics", true, "Collect metrics from log files.")
	flag.BoolVar(&collectNorthboundMetrics, "collect.northbound.metrics", true, "Collect metrics from OVN Northbound DB.")
	flag.BoolVar(&collectSouthboundMetrics, "collect.southbound.metrics", true, "Collect metrics from OVN Southbound DB.")
	flag.BoolVar(&collectNorthdMetrics, "collect.northd.metrics", true, "Collect metrics from ovn-northd.")
	...
	opts := ovn.Options{
		...
		EnabledFeatures: map[string]bool{
			"collectOvsdbServerMetrics": collectOvsdbServerMetrics,
			"collectVswitchMetrics":     collectVswitchMetrics,
			"collectPidMetrics":         collectPidMetrics,
			"collectLogMetrics":         collectLogMetrics,
			"collectNorthboundMetrics":  collectNorthboundMetrics,
			"collectSouthboundMetrics":  collectSouthboundMetrics,
		},
	}
